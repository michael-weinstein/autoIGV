...
Version 1.3: Cleaned up some of the error messages a bit and fixed the handling of filenames with whitespace in them.  This will require IGV 2.2.38
or later in order to work properly due to a bug in previous versions' handling of whitespaces in file and pathnames.  This version also remembers
bad bamfile paths and will avoid trying to load a bamfile during a run if it was previously unsuccessful (saves time and avoids repeated errors).
Finally, I set this version to capture the first image in a set of individual images at a locus while taking the group image (since the first load generates
the single )
        ...
        if stackshot and (multibam(locusarray, badbams) or not singleshot):  #if the user selected to get group photos of multiple bam files at each locus it will do this (if the user selected both multi and single image outputs and the line only had a single valid bam file, this will be skipped as both the multi and single shots would look the same)
            ...
            for i in range(1, len(locusarray)):
                ...
                if not firstshot and singleshot:
                    if cmdsaveimage(locusarray[i], locusarray[0], igv):
                        firstshot = True                       
